...
class DiseasePredictionModel:
    ...
    def predict(self, patient_data: dict) -> dict:
        ...
        if not self.is_trained or self.model is None or not self.feature_columns:
            raise ValueError("Model not trained or not loaded properly!")
    ...
    def _get_risk_factors(self, data: dict) -> list:
    ...
    def load(self, path: str) -> bool:
        """Load model from disk. Returns False if incompatible/corrupt pickle."""
        ...
        try:
            ...
            self.is_trained = bool(data.get("is_trained", True)) and (self.model is not None) and bool(self.feature_columns)
            return self.is_trained
        except Exception:
            # Most common in Streamlit Cloud: sklearn/joblib version mismatch
            return False